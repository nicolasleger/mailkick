module Mailkick
  class Processor
    ...
    def process
      ...
      parts.each do |part|
        # Avoid Frozen String change error from mail gem
        if part.body.raw_source.frozen?
          part.body(part.body.raw_source.gsub(/%7B%7BMAILKICK_TOKEN%7D%7D/, CGI.escape(token)))
        else
